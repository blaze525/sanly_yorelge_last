...
export function ShareRoadmapButton(props: ShareRoadmapButtonProps) {
  ...
  const embedHtml = `<iframe src="https://localhost/r/embed?id=${roadmapId}" width="100%" height="500px" frameBorder="0"\n></iframe>`;
    ...
    <div className="relative" ref={containerRef}>
      ...
      >
        {!isCopied && (
          <>
            <Share2 size="15px" />
            <span className="ml-2 hidden sm:inline">Paýlaşmak</span>
          </>
        )}
        {isCopied && (
          <>
            <Check size="15px" />
            <span className="ml-2 hidden sm:inline">Nusga almak</span>
          </>
        )}
      </button>
